var express = require('express');
var path = require('path');
var favicon = require('serve-favicon');
var logger = require('morgan');
var cookieParser = require('cookie-parser');
var bodyParser = require('body-parser');

var index = require('./routes/index');
var users = require('./routes/users');

var app = express();
const Web3 = require('web3');
var web3 = new Web3(new Web3.providers.WebsocketProvider('wss://ropsten.infura.io/ws'));

var io = require('socket.io')();

io.on('connection', function (client) {
    // client.emit('pendingTransactions','123')
});

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'ejs');

// uncomment after placing your favicon in /public
//app.use(favicon(path.join(__dirname, 'public', 'favicon.ico')));
app.use(logger('dev'));
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({extended: false}));
app.use(cookieParser());
app.use(express.static(path.join(__dirname, 'public')));

app.use('/', index);
app.use('/users', users);

function subscribe_pendingTransactions() {
    web3.eth.subscribe('pendingTransactions', function (error, result) {
        if (error) {
            console.log(error);
        }
    })
        .on("data", function (transaction) {
            web3.eth.getTransaction(transaction).then(object => {
                // if (object.from === '0x24C492A91eF42F34F1D38A51C7d4d8DfC9b7DC83') {
                //
                // }
                io.emit('pendingTransactions', object);
            });
        });
}

function subscribe_newBlockHeaders() {
    web3.eth.subscribe('newBlockHeaders', function (error, result) {
        if (error)
            console.log(error);
    })
        .on("data", function (blockHeader) {
            web3.eth.getBlockTransactionCount(blockHeader.number).then(object => {
                io.emit('newBlockHeaders', object);
            });
        });
}

setTimeout(() => {
    subscribe_pendingTransactions();
    subscribe_newBlockHeaders();
}, 2500);

function restart() {
    web3 = new Web3(new Web3.providers.WebsocketProvider('wss://ropsten.infura.io/ws'));
    subscribe_pendingTransactions();
    subscribe_newBlockHeaders();
}

setInterval(() => {
    // console.log("Restarting websocket connection.");
    restart()
}, 60 * 1000);

// catch 404 and forward to error handler
app.use(function (req, res, next) {
    var err = new Error('Not Found');
    err.status = 404;
    next(err);
});

// error handler
app.use(function (err, req, res, next) {
    // set locals, only providing error in development
    res.locals.message = err.message;
    res.locals.error = req.app.get('env') === 'development' ? err : {};

    // render the error page
    res.status(err.status || 500);
    res.render('error');
});

io.listen(3000);
module.exports = app;
